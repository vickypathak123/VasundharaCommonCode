...
import com.example.gallery.internal.utils.Utils;
...
public class AlbumMediaAdapter extends RecyclerViewCursorAdapter<RecyclerView.ViewHolder> implements MediaGrid.OnMediaGridClickListener {
    ...
    @Override
    public void onThumbnailClicked(ImageView thumbnail, Item item, RecyclerView.ViewHolder holder, boolean isPreview) {

        if (Utils.isNotValidImage(thumbnail)) {
            Toast.makeText(thumbnail.getContext(), thumbnail.getContext().getString(R.string.cannot_select_malicious_image), Toast.LENGTH_SHORT).show();
            return;
        }

        if (mSelectionSpec.showPreview || mSelectionSpec.maxSelectable == 1) {
            if (mOnMediaClickListener != null) {
                mOnMediaClickListener.onMediaClick(null, item, holder.getAdapterPosition(), isPreview);
            }
        } else {
    ...
    @Override
    public void onCheckViewClicked(CheckView checkView, Item item, RecyclerView.ViewHolder holder, ImageView thumbnail) {
        if (Utils.isNotValidImage(thumbnail)) {
            Toast.makeText(thumbnail.getContext(), thumbnail.getContext().getString(R.string.cannot_select_malicious_image), Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public interface OnMediaClickListener {
        void onMediaClick(Album album, Item item, int adapterPosition, boolean isPreview);
    }
